...
import common.testOnly.connectors.{TaxSummariesConnector, TaxSummariesStubsConnector}
import common.testOnly.forms.EnterODSFormProvider
import common.testOnly.models.CountryAndODSValues
import common.testOnly.views.html.EnterODSView
    ...
    taxSummariesStubsConnector.get(taxYear, utr).flatMap { saODSModel =>
      taxSummariesConnector.connectToAtsSaFields(taxYear).map {
        case Right(validOdsFieldNames) =>
          ...
          val submitCall: Call                         = common.testOnly.controllers.routes.EnterODSController.onSubmit(taxYear, utr)
          ...
          .fold(
            ...
            value => {
              ...
              taxSummariesStubsConnector.save(taxYear, utr, updatedValue).map { _ =>
                Redirect(common.testOnly.controllers.routes.DisplayPTAController.onPageLoad(taxYear, utr))
              }
            }
          )
      case Left(e)                   => throw e
