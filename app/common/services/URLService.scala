...
class URLService @Inject() (
  ...
  private val startPage = common.controllers.routes.SelectTaxYearController.onPageLoad

  private def safeEncodedUrl(url: String, hostAndPort: String): String =
    RedirectUrl(url).getEither(
      ...
      case Left(error)            =>
        ...
        URLEncoder.encode(s"$hostAndPort$startPage", "UTF-8")
    }
  ...
  def localFriendlyEncodedUrl(url: String, hostAndPort: String): String = {
    val isLocalEnv =
      if (env.mode.equals(Mode.Test)) {
        ...
        !appConfig.getOptional[String]("run.mode").contains(Mode.Prod.toString)
      }
