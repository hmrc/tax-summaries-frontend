...
trait ErrorMessageInterpreter {
  ...
  def getFieldErrors(field: Field, parent: Option[Field] = None)(
    implicit form: Option[Form[_]] = None,
    messages: Messages): Seq[FieldError]
...
object ErrorMessageInterpreter extends ErrorMessageInterpreter {
  ...
  private def extract(str: String)(implicit messages: Messages): MessageArguments = {
    ...
    if (close != -1) {
      ...
      val processed = str.substring(0, open) + processEmbedded(innerContent) + str
        .substring(close + embeddedEnd.length, str.length)
      extract(processed)
    } else {
      extractParam(str)
    }
  ...
  private def paramContainsId(id: String, args: Seq[Any]): Boolean =
    args
      .map { x =>
        x match {
  ...
  def getFieldErrors(field: String, form: Form[_])(implicit messages: Messages): Seq[FieldError] = {
    lazy val filtered = form.errors.filter { error =>
      error.key == field || error.args.contains(field) || field == error.args.fold(error.key) {
        _ + "." + _
      }
    }
    filtered.map(error => formatFieldError(error))
  ...
  private def getAnchorId(args: Seq[Any]): String =
    args.filter { arg =>
      arg match {
  ...
  private def extract2(str: String)(implicit messages: Messages): MessageArguments = {
    ...
    if (close != -1) {
      val open: Int = str.substring(0, close).lastIndexOf(embeddedStart)
      val innerContent: String = str.substring(open + embeddedStart.length, close)
      val processed = str.substring(0, open) + processSummaryEmbedded(innerContent) + str
        .substring(close + embeddedEnd.length, str.length)
      extract2(processed)
    } else {
  ...
  private def SummaryParam(errMsg: String)(implicit messages: Messages): MessageArguments =
    errMsg contains (summaryIdMarker) match {
      case true =>
        errMsg.split(fieldDelimiter, -1).head.split(summaryIdMarker, -1).drop(1) match {
