...
class IndexController @Inject()(dataCacheConnector: DataCacheConnector,
                                atsYearListService: AtsYearListService,
                                atsListService: AtsListService,
                                val auditService: AuditService,
                                authAction: AuthAction) extends TaxsController {

  implicit val formPartialRetriever: FormPartialRetriever = AppFormPartialRetriever

  def authorisedIndex: Action[AnyContent] = authAction.async {
    request => agentAwareShow(request)
  }

  def authorisedOnSubmit: Action[AnyContent] = authAction.async {
    ...
    request.getQueryString(Globals.TAXS_USER_TYPE_QUERY_PARAMETER) match {
      case Some(Globals.TAXS_PORTAL_REFERENCE) => {
        ...
        agentToken.fold[Future[_]] {
          ...
          _ => Redirect(routes.IndexController.authorisedIndex()).withSession(session)
        }
      }
      case _ => show(request)
    }
