    ...
    val constraintOdsValues: Constraint[String] = Constraint { odsValues =>
      ...
      if (duplicatedFields.nonEmpty) {
        ...
        if (unrecognisedFields.isEmpty) {
          ...
          val unrecognisedFieldsIgnoringCase =
            unrecognisedFields.map(_.toLowerCase).diff(validOdsFieldNames.map(_.toLowerCase))
          if (unrecognisedFieldsIgnoringCase.isEmpty) {
            keyValuePairsToEitherSeqODSValue(keyValuePairs) match {
              case Left(invalidFields) => Invalid(s"Invalid field values: ${invalidFields.sorted.mkString(", ")}")
              case Right(_)            => Valid
            }
          } else {
            Invalid(s"Unrecognised field names: ${unrecognisedFieldsIgnoringCase.sorted.mkString(", ")}")
          }
        }
      }
    }
