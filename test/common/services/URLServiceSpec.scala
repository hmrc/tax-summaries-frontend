...
class URLServiceSpec extends BaseSpec {
  ...
  private val prodEnv: Environment  = Environment(new File(""), classOf[URLServiceSpec].getClassLoader, Mode.Prod)
  ...
  "localFriendlyUrl" should {
    ...
    "return url string with localhost and port if is in development (local/ jenkins) environment" in {
      ...
      when(mockApplicationConfig.getOptional[String](any())(any())).thenReturn(Some("Dev"))
      service.localFriendlyEncodedUrl("/A?a=b&c=d", "localhost") mustBe "http%3A%2F%2Flocalhost%2FA%3Fa%3Db%26c%3Dd"
    }

    "return url string with localhost and port if is not set (local/ jenkins) environment" in {
      val service = new URLService(mockApplicationConfig, devEnv)
      when(mockApplicationConfig.getOptional[String](any())(any())).thenReturn(None)
      service.localFriendlyEncodedUrl("/A?a=b&c=d", "localhost") mustBe "http%3A%2F%2Flocalhost%2FA%3Fa%3Db%26c%3Dd"
    }

    "return the original url if is in production environment" in {
      val service = new URLService(mockApplicationConfig, prodEnv)
      when(mockApplicationConfig.getOptional[String](any())(any())).thenReturn(Some("Prod"))
      service.localFriendlyEncodedUrl("/A?a=b&c=d", "B") mustBe "%2FA%3Fa%3Db%26c%3Dd"
    }
    ...
    "if url is not an accepted url regardless of environment" in {
      val service = new URLService(mockApplicationConfig, devEnv)
      when(mockApplicationConfig.getOptional[String](any())(any())).thenReturn(Some("Prod"))
      service.localFriendlyEncodedUrl("http://A?a=b&c=d", "B") mustBe "B%2Fannual-tax-summary"
    }
